...
- 技術スタックは Next.js / TypeScript / Tailwind CSS / Supabase / OpenAI API / Vercel を前提にする。
...
- OpenAI生成処理は `features/ai/providers` に閉じ込め、モック生成と本番生成を切り替え可能にする。
...
- OpenAI APIエラー、JSON parseエラー、バリデーションエラーを分けて保存する。
...
- OpenAI API障害時にMVP全体が止まらないよう、mock生成や再試行を用意する。
